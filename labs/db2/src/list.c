...
{
    if (!strcmp(s,"running"))  { *out_s = running;  return 1; }
    if (!strcmp(s,"ready"))    { *out_s = ready;    return 1; }
    if (!strcmp(s,"paused"))   { *out_s = paused;   return 1; }
    if (!strcmp(s,"blocked"))  { *out_s = blocked;  return 1; }
    if (!strcmp(s,"dying"))    { *out_s = dying;    return 1; }
    if (!strcmp(s,"sleeping")) { *out_s = sleeping; return 1; }
    return 0;
}
...
static int parse_time(const char *s, time_t *t)
{
    struct tm tm = {0};
    int d, mo, y, h = 0, mi = 0;
    if (sscanf(s, "%d.%d.%d_%d:%d", &d, &mo, &y, &h, &mi) < 3) return 0;
...
{
    if      (!strcmp(f,"pid"))      snprintf(buf, sz, "%d",   p->pid);
    else if (!strcmp(f,"name"))     snprintf(buf, sz, "%s",   p->name);
    else if (!strcmp(f,"priority")) snprintf(buf, sz, "%d",   p->priority);
    else if (!strcmp(f,"kern_tm"))  fmt_time(p->kern_tm, buf, sz);
    else if (!strcmp(f,"file_tm"))  fmt_time(p->file_tm, buf, sz);
    else if (!strcmp(f,"cpu_usage"))snprintf(buf, sz, "%.2f", p->cpu_usage);
    else if (!strcmp(f,"status"))   snprintf(buf, sz, "%s",   status_str(p->status));
...
static int set_field(process *p, const char *f, const char *v)
{
    if (!strcmp(f,"pid")) {
        ...
        strncpy(p->name, v, MAX_NAME-1); p->name[MAX_NAME-1] = '\0';
    } else if (!strcmp(f,"priority")) {
        char *e; long n = strtol(v, &e, 10);
        if (*e) return 0;
        p->priority = (int)n;
    } else if (!strcmp(f,"kern_tm")) {
        if (!parse_time(v, &p->kern_tm)) return 0;
    } else if (!strcmp(f,"file_tm")) {
        if (!parse_time(v, &p->file_tm)) return 0;
    } else if (!strcmp(f,"cpu_usage")) {
        char *e; float n = strtof(v, &e);
        if (*e) return 0;
        p->cpu_usage = n;
    } else if (!strcmp(f,"status")) {
        if (!str_to_status(v, &p->status)) return 0;
    } else {
        return 0;
    }
    return 1;
...
static int eval_cond(const process *p, const cond *c) {
    ...
    if (!strcmp(c->f,"pid") || !strcmp(c->f,"priority")) {
        ...
        time_t ta, tb;
        if (!parse_time(fval, &ta) || !parse_time(c->v, &tb)) return 0;
        cmp = (ta > tb) - (ta < tb);
...
static int eval_all(const process *p, cond *conds, int n) {
    for (int i = 0; i < n; i++)
        if (!eval_cond(p, &conds[i])) return 0;
    return 1;
}
...
static void print_record(FILE *f, const process *p,
    ...
    for (int i = 0; i < nnames; i++) {
        char v[256]; get_field(p, names[i], v, sizeof(v));
